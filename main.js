/* ================================================================
   SOLO AI ARCHITECT ACADEMY — MAIN JS
...
const PHASES = [
  ...
  {
    id:'7.5', icon:'🧠', shortTitle:'Frontier Models & Alignment', tag:'Optional · Frontier AI', optional:true, parentId:7, nextId:8,
    title:'Frontier Foundation Models & Alignment Engineering',
    goal:'Move beyond basic GPT architectures into the exact post-training, alignment, and large-scale model mechanisms powering modern frontier AI systems.',
    color:'#00c8ff',
    topics:[
      {
        number:'1', title:'Modern Transformer Evolution',
        desc:'Traditional GPT architectures are no longer sufficient for state-of-the-art systems. Modern systems improve efficiency, scalability, long-context understanding, and reasoning through advanced architectural modifications.',
        bullets:[
          '<strong>Rotary Positional Embeddings (RoPE):</strong> Injecting relative positional information into token representations while preserving long-range contextual relationships.',
          '<strong>Grouped Query & Multi-Query Attention (GQA/MQA):</strong> Reducing memory overhead by sharing key-value representations across attention heads while maintaining quality, and lowering inference latency for large-scale production systems.',
          '<strong>Mixture of Experts (MoE) & Sparse Attention:</strong> Activating only selected subnetworks for incoming tokens to massively reduce computational cost while increasing parameter capacity, and computing attention selectively rather than across all tokens to improve scaling.',
          '<strong>State Space Models (Mamba):</strong> Replacing traditional quadratic attention mechanisms with efficient sequence-state representations for long-context processing.'
        ],
        courseUrl:'https://www.youtube.com/playlist?list=PLAqhIrjkxbuWI23v9cThsA9GvCAUhRvKZ',
        courseName:'Andrej Karpathy: Neural Networks — Zero to Hero (video playlist)'
      },
      {
        number:'2', title:'Alignment & Human Feedback Learning',
        desc:'Pre-trained language models understand language but do not automatically align with human preferences, safety requirements, or intended behavior. Study this video course across three passes: Pass 1 for conceptual understanding, Pass 2 to rebuild all code manually without looking at solutions, Pass 3 to read assignments and implementation details.',
        bullets:[
          '<strong>Reinforcement Learning Foundations & Reward Modeling:</strong> Learning policies that maximize reward signals over long-term actions, and constructing neural systems capable of learning human preference patterns.',
          '<strong>RLHF & PPO:</strong> Optimizing language model behavior according to human evaluations (Reinforcement Learning From Human Feedback), and stabilizing policy updates during reinforcement training with Proximal Policy Optimization.',
          '<strong>DPO & Constitutional AI:</strong> Training preference-aligned systems without explicit reinforcement loops via Direct Preference Optimization, and training models to follow high-level behavioral principles for safety alignment and controllability.'
        ],
        courseUrl:'https://cs336.stanford.edu/',
        courseName:'Stanford CS336: Language Modeling From Scratch (video course + assignments)'
      },
      {
        number:'3', title:'Advanced LLM Inference Optimization',
        desc:'Large models become useless if they generate slowly or consume excessive GPU memory. Study using a 40% watching and note-taking / 60% manual implementation and debugging split across both linked video resources.',
        bullets:[
          '<strong>KV Cache & Continuous Batching:</strong> Storing previous attention computations to avoid recomputation, and dynamically grouping incoming user requests during generation for production throughput.',
          '<strong>Speculative Decoding & Prefix Caching:</strong> Using lightweight draft models to accelerate generation, and reusing shared prompt segments across users to eliminate redundant computation.',
          '<strong>Flash Attention & Memory-Efficient Architectures:</strong> Optimizing GPU memory access during attention operations to unlock longer context windows without proportional memory cost.'
        ],
        courseUrl:'https://www.youtube.com/playlist?list=PLQRyiBCWmqp5twpd8Izmaxu5XRkxd5yC-',
        courseName:'Sebastian Raschka: Build Large Language Models From Scratch (video playlist)'
      },
      {
        number:'4', title:'Elite-Level Research Material — Not Beginner Friendly',
        desc:'Two foundational RLHF research papers. Read in this exact order: Step 1 — read the Abstract. Step 2 — read the Introduction. Step 3 — study the architecture diagrams. Step 4 — read the Conclusions. Step 5 — return later and understand the mathematical sections. Do not attempt line-by-line reading initially; extract ideas before studying equations.',
        bullets:[
          '<strong>Deep Reinforcement Learning From Human Preferences:</strong> The foundational paper establishing how human preference comparisons can shape reinforcement learning policies at scale.',
          '<strong>Training a Helpful and Harmless Assistant with RLHF:</strong> The applied research paper detailing how RLHF is used to train safety-aligned, helpful conversational assistants.'
        ],
        courseUrl:'https://arxiv.org/abs/1706.03741',
        courseName:'Research Papers (arXiv): Deep RL From Human Preferences + Training a Helpful and Harmless Assistant with RLHF'
      }
    ],
    projects:[
      {type:'foundation', title:'Modern Attention Architecture Laboratory',
       what:'A modular experimentation framework that implements and compares multiple modern attention mechanisms used by frontier language models.',
       desc:'Build and compare Standard Multi-Head Attention, Grouped Query Attention (GQA), Multi-Query Attention (MQA), and Flash Attention. Feed identical text inputs into all four architectures, measure GPU memory consumption, token generation speed, latency per request, and throughput under increasing loads, visualize attention behavior using graphs, and generate benchmark reports comparing all methods. Tests understanding of modern transformer internals, attention mathematics, optimization strategies, and performance engineering.'},
      {type:'capstone', title:'Human Preference-Aligned Mini Foundation Model',
       what:'A complete decoder-only miniature foundation model and post-training ecosystem that simulates how modern frontier AI systems are aligned with human preferences.',
       desc:'Build a custom tokenizer, a miniature GPT architecture, a preference dataset generator, a reward model, an RLHF training pipeline, a DPO optimization pipeline, a safety alignment layer, and a response evaluation engine. Train the model on a text corpus, collect preference comparisons, optimize outputs using RLHF, compare aligned vs. non-aligned responses, measure hallucination reduction, measure safety improvements, deploy through FastAPI, and create a monitoring dashboard. Goal output: a complete aligned miniature foundation model capable of generating safer and more controllable outputs using real-world post-training methods.'}
    ]
  },
  {
  ...
  {
    id:'10.5', icon:'⚡', shortTitle:'Distributed AI Infrastructure', tag:'Optional · GPU Clusters', optional:true, parentId:10, nextId:11,
    title:'Distributed AI Infrastructure & Large-Scale Model Training',
    goal:'Learn the exact distributed systems and GPU infrastructure used to train modern large-scale foundation models across multiple machines and GPU clusters.',
    color:'#a855f7',
    topics:[
      {
        number:'1', title:'Large-Scale Distributed Training Systems',
        desc:'Single-device training eventually becomes impossible as model parameters and datasets grow into billions of tokens and billions of parameters. Study across three passes using all three linked resources: Pass 1 to understand concepts and architecture, Pass 2 to reproduce examples manually, Pass 3 to modify implementations without references.',
        bullets:[
          '<strong>Data, Tensor & Pipeline Parallelism:</strong> Splitting datasets across multiple GPUs while synchronizing parameter updates, splitting neural network layers across multiple GPUs, and dividing network layers into separate sequential execution stages.',
          '<strong>Sequence Parallelism & Gradient Checkpointing:</strong> Distributing token sequence processing across hardware resources, and trading computation for memory to train models larger than GPU memory limits.',
          '<strong>Fully Sharded Data Parallel (FSDP) & ZeRO Optimization:</strong> Sharding parameters, gradients, and optimizer states across GPUs, and reducing redundant memory allocation during large-scale training.',
          '<strong>DeepSpeed & Megatron-LM:</strong> Industry-standard frameworks for optimizing distributed training memory usage and large-scale transformer training infrastructure.'
        ],
        courseUrl:'https://pytorch.org/tutorials/intermediate/ddp_tutorial.html',
        courseName:'PyTorch: Distributed Training Official Tutorials'
      },
      {
        number:'2', title:'GPU Cluster Architecture',
        desc:'Training large models requires understanding how GPUs communicate and coordinate at massive scale. Focus on concepts rather than memorization, and draw architecture diagrams manually while studying both linked resources.',
        bullets:[
          '<strong>NCCL Communication & Multi-GPU Synchronization:</strong> High-speed GPU-to-GPU communication protocols, and coordinating training across distributed devices without state divergence.',
          '<strong>Cluster Topology & GPU Scheduling:</strong> Understanding network arrangements among GPU systems, and allocating workloads across available hardware efficiently.',
          '<strong>GPU Memory Bottlenecks & Compute Throughput Optimization:</strong> Identifying performance limitations and systematically maximizing sustained compute throughput.'
        ],
        courseUrl:'https://www.youtube.com/playlist?list=PLQnljOFTspQU0ICDe-cL1EwXC4GDSayKY',
        courseName:'Hussein Nasser: Full Distributed Systems Playlist'
      },
      {
        number:'3', title:'Training Stability Engineering',
        desc:'Large models become unstable during training without proper optimization and monitoring.',
        bullets:[
          '<strong>Gradient Explosion & Vanishing:</strong> Diagnosing and correcting the two classic failure modes that destroy training runs mid-flight.',
          '<strong>Learning Rate Scheduling & Mixed Precision Training:</strong> Tuning warmup and decay schedules for stability, and using reduced-precision arithmetic with loss scaling to accelerate training without numerical collapse.',
          '<strong>Training Failure Diagnosis & Numerical Stability:</strong> Systematically identifying root causes of divergence and applying memory optimization strategies to keep large runs stable end-to-end.'
        ],
        courseUrl:'https://www.coursera.org/learn/deep-neural-network',
        courseName:'DeepLearning.AI: Training Deep Neural Networks (video course)'
      }
    ],
    projects:[
      {type:'foundation', title:'Distributed Training Architecture Simulator',
       what:'A distributed experimentation environment for understanding how large neural networks are trained across multiple GPUs.',
       desc:'Implement and compare Single GPU training, Data Parallelism, Tensor Parallelism, and Pipeline Parallelism. Train identical models while monitoring GPU utilization, communication overhead, memory consumption, and throughput, measure scaling efficiency, visualize performance metrics, and generate benchmark reports. Tests understanding of distributed training concepts, GPU coordination, communication bottlenecks, and scaling behavior.'},
      {type:'capstone', title:'Distributed Foundation Model Training Framework',
       what:'A production-grade distributed training engine capable of handling large transformer architectures.',
       desc:'Build distributed dataloaders, an FSDP implementation, DeepSpeed optimization, ZeRO optimization, mixed precision training, gradient checkpointing, a GPU monitoring dashboard, and a training checkpoint recovery system. The system should distribute training across multiple GPUs, recover automatically after failures, optimize memory consumption, track cluster performance, monitor training metrics, and generate benchmark reports. Goal output: a scalable training infrastructure capable of handling large-scale transformer workloads efficiently.'}
    ]
  },
  {
  ...
  {
    id:'12.5', icon:'🖥️', shortTitle:'GPU Systems, CUDA & Optimization', tag:'Optional · Low-Level GPU', optional:true, parentId:12, nextId:13,
    title:'GPU Systems, CUDA & Low-Level AI Optimization',
    goal:'Understand the hardware and computational systems beneath AI frameworks and learn how modern large-scale AI systems maximize GPU performance.',
    color:'#f97316',
    topics:[
      {
        number:'1', title:'GPU Architecture & Parallel Computing Fundamentals',
        desc:'Modern AI models rely on highly parallel hardware systems that process thousands of operations simultaneously. Understanding the hardware layer is essential for building highly optimized AI systems. Study across three passes: Pass 1 to understand concepts visually, Pass 2 to draw architecture diagrams manually, Pass 3 to explain concepts without notes.',
        bullets:[
          '<strong>GPU vs. CPU Architecture:</strong> Understanding why GPUs dominate deep learning workloads through massive parallelism versus CPU sequential optimization.',
          '<strong>GPU Memory Hierarchy & Tensor Cores:</strong> Global memory, shared memory, cache memory, and registers, plus the specialized hardware units for accelerated matrix operations.',
          '<strong>Thread Blocks, Warp Execution & Bottlenecks:</strong> Understanding GPU execution scheduling and identifying the performance constraints that silently cap real-world throughput.'
        ],
        courseUrl:'https://developer.nvidia.com/blog/even-easier-introduction-cuda/',
        courseName:'NVIDIA: An Even Easier Introduction to CUDA (documentation + tutorial)'
      },
      {
        number:'2', title:'CUDA Programming & Kernel Development',
        desc:'AI frameworks internally execute highly optimized CUDA operations. Understanding these systems allows custom optimization beyond framework limitations. Study using a 40% watching / 60% implementation split — do not proceed until every CUDA example works manually.',
        bullets:[
          '<strong>CUDA Programming Model & Thread Hierarchies:</strong> Writing CUDA Kernels directly and structuring thread hierarchies for correct, efficient parallel execution.',
          '<strong>Shared Memory Optimization & Memory Coalescing:</strong> Structuring memory access patterns for maximum bandwidth utilization on real hardware.',
          '<strong>Kernel Fusion & Asynchronous GPU Execution:</strong> Combining operations to reduce launch overhead, and overlapping compute with data transfer for maximum throughput.',
          '<strong>Performance Profiling:</strong> Using profiling tools to identify exactly which kernel is the true bottleneck before optimizing it.'
        ],
        courseUrl:'https://www.youtube.com/watch?v=n6AKatqzN8U',
        courseName:'FreeCodeCamp: CUDA Programming Course (video)'
      },
      {
        number:'3', title:'Triton & AI Kernel Optimization',
        desc:'Modern AI systems increasingly rely on custom optimized kernels rather than generic framework operations. When using the documentation: Step 1 — read the explanation section. Step 2 — run the code examples. Step 3 — modify code behavior manually. Step 4 — benchmark against the original implementation.',
        bullets:[
          '<strong>Triton Programming Fundamentals:</strong> Writing GPU kernels in a Python-like syntax that compiles to highly optimized machine code.',
          '<strong>Matrix Multiplication & Flash Attention Optimization:</strong> Implementing the two operations that dominate transformer compute cost, with memory-efficient attention variants.',
          '<strong>Kernel-Level Optimization & High-Throughput Inference:</strong> Applying systematic optimization strategies to push production inference engines to their hardware ceiling.'
        ],
        courseUrl:'https://triton-lang.org/main/getting-started/tutorials/index.html',
        courseName:'OpenAI Triton Tutorials (official documentation)'
      }
    ],
    projects:[
      {type:'foundation', title:'GPU Kernel Optimization Laboratory',
       what:'Custom computational operations compared against low-level GPU execution performance from existing frameworks.',
       desc:'Implement and compare NumPy operations, PyTorch operations, and custom CUDA operations. Execute matrix operations while measuring execution speed, monitoring GPU utilization, monitoring memory usage, comparing throughput, visualizing performance differences, and generating optimization reports. Tests understanding of GPU architecture, CUDA fundamentals, performance optimization, and parallel computation.'},
      {type:'capstone', title:'Custom Flash Attention & Kernel Optimization Engine',
       what:'A highly optimized transformer computation engine using custom CUDA and Triton kernels.',
       desc:'Build custom CUDA kernels, Triton kernels, optimized matrix multiplication, and memory-efficient attention operations. Benchmark against PyTorch\'s native attention, compare memory consumption, compare tokens-per-second generation speed, monitor GPU utilization, profile bottlenecks, generate optimization reports, and create a real-time performance dashboard. Goal output: a highly optimized transformer attention engine capable of executing large-scale inference workloads with significantly improved speed and memory efficiency.'}
    ]
  },
  {
  ...
  {
    id:'14.5', icon:'🧠', shortTitle:'Elite Research & Foundation Model Lab', tag:'Optional · Research Frontier', optional:true, parentId:14, nextId:null,
    title:'Elite Research Engineering & Foundation Model Laboratory',
    goal:'Transition from implementing existing AI systems to designing, experimenting with, and creating new frontier AI architectures and research workflows.',
    color:'#ff4fcb',
    topics:[
      {
        number:'1', title:'Research Methodology & Experimental Design',
        desc:'Engineering alone is insufficient at the frontier level. Modern AI progress requires disciplined experimentation, scientific reasoning, reproducible workflows, and systematic analysis. Study across three passes: Pass 1 to watch for conceptual understanding, Pass 2 to summarize every lecture using your own words, Pass 3 to convert major concepts into practical experiments.',
        bullets:[
          '<strong>Reading Research Papers Efficiently:</strong> Extracting useful implementation knowledge and architectural insights without becoming overwhelmed by dense mathematical notation.',
          '<strong>Experimental Design & Benchmark Systems:</strong> Creating controlled experiments with measurable variables and repeatable procedures, and comparing multiple architectures using standardized evaluation metrics.',
          '<strong>Ablation Studies & Hypothesis Development:</strong> Measuring exactly how individual model components contribute to performance, and forming testable assumptions before implementation begins.',
          '<strong>Statistical Validation & Failure Analysis:</strong> Verifying experimental results mathematically, and understanding why systems fail to design disciplined improvement cycles.'
        ],
        courseUrl:'https://www.youtube.com/playlist?list=PLUl4u3cNGP63_j6wJ2SEggxmcBQa1Izhr',
        courseName:'MIT Deep Learning: Research Lectures (video playlist)'
      },
      {
        number:'2', title:'Foundation Model Research Systems',
        desc:'Modern frontier AI systems emerge from scaling laws, architecture experimentation, and extensive model behavior analysis. Do not simply watch these lectures — manually implement every major architecture component discussed.',
        bullets:[
          '<strong>Scaling Laws & Emergent Behaviors:</strong> Understanding how model performance changes with parameters, compute, and data, and studying capabilities that appear unexpectedly at larger scales.',
          '<strong>Synthetic Data & Retrieval-Augmented Training:</strong> Producing artificial training data to improve model capabilities, and combining retrieval systems directly into training workflows.',
          '<strong>Multimodal & Self-Improving Systems:</strong> Jointly learning from text, images, audio, and additional modalities, and designing systems capable of iterative self-enhancement.',
          '<strong>Evaluation Frameworks & Model Benchmarking:</strong> Building rigorous, standardized systems for comparing frontier model capabilities across releases.'
        ],
        courseUrl:'https://cs336.stanford.edu/',
        courseName:'Stanford CS336: Language Modeling From Scratch (video course + assignments)'
      },
      {
        number:'3', title:'Research Workflow Engineering',
        desc:'Research becomes ineffective without structured workflows, reproducibility systems, and experiment tracking. Never execute experiments without recording: Parameters, Metrics, Dataset Versions, Model Versions, Hardware Information, and Observations.',
        bullets:[
          '<strong>Experiment Tracking & Hyperparameter Optimization:</strong> Systematically logging every training run, and searching the hyperparameter space in a reproducible, comparable way.',
          '<strong>Dataset & Model Versioning:</strong> Treating data and weights with the same rigor as source code so any result can be traced back to its exact inputs.',
          '<strong>Reproducibility Engineering & Automated Benchmark Pipelines:</strong> Building systems where any experiment can be re-run and verified by another engineer, automatically.',
          '<strong>Failure Analysis & Research Logging Systems:</strong> Maintaining disciplined logs of what did not work — the single most valuable and most commonly discarded research artifact.'
        ],
        courseUrl:'https://www.youtube.com/@weightsbiases',
        courseName:'Weights & Biases: Tutorial Series (video channel)'
      },
      {
        number:'4', title:'Elite-Level Research Material — Not Beginner Friendly',
        desc:'Three landmark research papers that define the modern AI era. Read in this exact order: Step 1 — read the Abstract. Step 2 — read the Introduction. Step 3 — study the architecture diagrams and figures. Step 4 — read the Conclusions. Step 5 — return later and understand the mathematical sections. Do not attempt line-by-line reading initially; extract ideas before studying equations.',
        bullets:[
          '<strong>Attention Is All You Need:</strong> The original Transformer paper — the architectural foundation underlying every modern frontier language model.',
          '<strong>Scaling Laws for Neural Language Models:</strong> The paper establishing the mathematical relationships between model size, dataset size, compute, and performance that guide every frontier lab\'s training decisions.',
          '<strong>LoRA — Low-Rank Adaptation of Large Language Models:</strong> The paper introducing the parameter-efficient fine-tuning technique now standard across the entire industry.'
        ],
        courseUrl:'https://arxiv.org/abs/1706.03762',
        courseName:'Research Papers (arXiv): Attention Is All You Need + Scaling Laws for Neural Language Models + LoRA'
      }
    ],
    projects:[
      {type:'foundation', title:'Research Paper Reproduction Laboratory',
       what:'Selecting a modern AI research paper and reproducing its core implementation from scratch.',
       desc:'Read and analyze the research paper, rebuild the model architecture, train using the original methodology, compare results against published benchmarks, analyze performance differences, generate reproducibility reports, and create documentation explaining findings. Tests understanding of research comprehension, implementation skills, experimentation methodology, and analytical thinking.'},
      {type:'capstone', title:'Foundation Model Research Ecosystem',
       what:'A complete research-grade AI ecosystem designed for experimentation, evaluation, and large-scale model development.',
       desc:'Build a tokenizer system, a distributed training pipeline, an alignment system, RAG integration, a multi-agent framework, an experiment tracking pipeline, a monitoring dashboard, a security testing system, and deployment infrastructure. The system should track experiments automatically, benchmark model behavior, evaluate performance metrics, compare multiple architectures, generate visual reports, and create technical documentation. Goal output: a complete research-grade AI laboratory capable of designing, testing, evaluating, and improving modern foundation model systems.'}
    ]
  }
];
...
function sendContactMessage() {
  ...
  const subject = encodeURIComponent(`SOLO AI Architect Academy Inquiry — ${name}`);
  const body    = encodeURIComponent(
    `Name: ${name}\nEmail: ${email}\n\n──────────────────\nMessage:\n${message}\n──────────────────\n\nSent via SOLO AI Architect Academy`
  );
...
function renderHome() {
  ...
  <section class="phases-section">
    ...
    <div class="phases-grid">
      ${PHASES.map(p => `
        <a class="phase-card" style="${p.optional?'border-color:rgba(168,85,247,0.35);background:linear-gradient(135deg,rgba(0,200,255,0.04),rgba(168,85,247,0.06))':''}" onclick="Router.navigate('phase',{id:${typeof p.id==='string'?`'${p.id}'`:p.id}})" href="javascript:void(0)" role="button" aria-label="Open Phase ${p.id}: ${p.shortTitle}">
          <div class="phase-card-glow"></div>
          <div class="phase-number">Phase ${p.optional ? p.id : String(p.id).padStart(2,'0')}${p.optional?' · Optional':''}</div>
...
function renderPhasePage(id) {
  ...
  <div class="page-hero">
    ...
    <div class="phase-hero-tag">${p.icon} Phase ${p.optional ? p.id : String(p.id).padStart(2,'0')} · ${p.tag}</div>
    ${p.optional ? `<div style="display:inline-flex;align-items:center;gap:8px;padding:6px 16px;background:linear-gradient(135deg,rgba(168,85,247,0.14),rgba(255,79,203,0.10));border:1px solid rgba(168,85,247,0.32);border-radius:24px;font-family:var(--font-mono);font-size:11px;color:var(--accent-purple);font-weight:700;letter-spacing:.04em;margin-bottom:18px">⭐ Optional Elite-Level Phase — Not required to progress, recommended for frontier-level mastery</div>` : ''}
  ...
  <div style="padding:0 32px;max-width:900px;margin:0 auto 16px">
    <div class="projects-section-btn reveal" onclick="Router.navigate('projects',{id:${typeof p.id==='string'?`'${p.id}'`:p.id}})">
  ...
  <div style="max-width:900px;margin:0 auto;padding:16px 32px 60px;display:flex;gap:12px;flex-wrap:wrap">
    ${p.optional
      ? `<button class="btn-secondary" onclick="Router.navigate('phase',{id:${p.parentId}})">← Phase ${p.parentId}</button>
         ${p.nextId ? `<button class="btn-primary" onclick="Router.navigate('phase',{id:${p.nextId}})">Phase ${p.nextId} →</button>` : ''}`
  ...
  <footer class="footer">
    <div class="footer-brand">SOLO AI Architect Academy</div>
    <p>${p.optional ? `Optional Phase ${p.id} · ${p.tag}` : `Phase ${p.id} of 14 · ${p.tag}`}</p>
  </footer>`;
}
...
function renderProjectsPage(id) {
  ...
  return `
  <div class="page-hero">
    <button class="back-btn" onclick="Router.navigate('phase',{id:${typeof p.id==='string'?`'${p.id}'`:p.id}})">← Back to Phase ${p.id}</button>
...
function renderAboutPage() {
  ...
  <div class="about-hero">
    ...
    <p style="font-size:16px;color:var(--text-secondary);line-height:1.7;max-width:620px;margin:18px auto 0">
      SOLO AI Architect Academy is not a course platform. It is an engineered execution system — a zero-to-architect 
